...
public class PolygonListener extends DrawListener {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if ((x[n - 1] - x[0]) * (x[n - 1] - x[0]) + (y[n - 1] - y[0]) * (y[n - 1] - y[0]) <= 64 && n > 2) {
            x[n - 1] = x[0];
            y[n - 1] = y[0];
            Polygon polygon = new Polygon(x, y, n);
            getListenerPen().drawImage(getDrawBoard().getDrawBoardCopy(), 0, 0, null);  //最后一次选点多画的线删掉
            super.getListenerPen().draw(polygon);
            super.getListenerPenCopy().draw(polygon);
            n = 0;
        }
